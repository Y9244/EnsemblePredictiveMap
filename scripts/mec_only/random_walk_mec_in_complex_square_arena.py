...
if __name__ == "__main__":
    ...
    for t in range(1, T):
        ...
        if t % elapsed_time == 0:
            ...
            #visualize_map.show_place_fields_in_square_arena(sc_place_fields, name=f'complex_env/MEC_sc_{t}')

            pmap_place_fields = visualize_map.compute_pmap_place_fields(pmap, sc_place_fields)
            visualize_map.show_place_fields_in_square_arena(pmap_place_fields, name=f'complex_env/MEC_pmap_{t}')
            ...
            visualize_map.show_reward_vector_in_square_arena(
                ...
                place_center_index,
                R,
                name=f'complex_env/MEC_R_{t}'
            )
            visualize_map.show_value_map_in_square_arena(
                pmap_place_fields, R,
                place_centers,
                place_center_index,
                name=f'complex_env/MEC_Vmap_{t}'
            )