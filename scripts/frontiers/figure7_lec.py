...
if __name__ == "__main__":
    ...
    sc_cfg = SparseCodingConfig(N_hip=N_h, N_mec=N_mec, N_lec=reward_cell.N_reward_cell)
    sparse_coding = SparseCoding(sc_cfg)

    pmap_cfg = PmapConfig(N_hip=N_h)
    pmap = Pmap(pmap_cfg)

    visualize_map = VisualizeMap(env=env, grid_cell=grid_cell, boundary_cell=boundary_cell, reward_cell=reward_cell, resolution=128)
    ...
    for t in range(1, T):
        ...
        if t % elapsed_time == 0:
            ...
            with open('../../data/Frontiers/figure7_lec.pkl', 'wb') as f:
