...
from config.default_config import (
    ...
    RewardCellConfig, SparseCodingConfig, PmapConfig
)
...
if __name__ == "__main__":
    ...
    sc_cfg = SparseCodingConfig(N_hip=N_h, N_mec=N_mec, N_lec=N_reward_cell)
    sparse_coding = SparseCoding(sc_cfg)

    pmap_cfg = PmapConfig(N_hip=N_h)
    pmap = Pmap(pmap_cfg)

    visualize_map = VisualizeMap(env=env, grid_cell=grid_cell, boundary_cell=boundary_cell,
                                 reward_cell=reward_cell, resolution=128)

    action = np.zeros(2)
    s_e, _, s_h, p_h = one_step_EC(action, env, sparse_coding, pmap)
    for t in range(1, T):
        ...
        agent.learning_policy(reward_vector, s_h, next_s_h, p_h, next_p_h,
                              R_replay=True, episode_i=None, t=t)
