...
if __name__ == "__main__":
    ...
    i_cells = rng.integers(low=0, high=BoundaryCellConfig.N_cells, size=2*N_cell)
    ...
    for index in range(2*N_cell):
        i_cell = i_cells[index]
        i = index // 10
        j = index % 10
        print(i_cell)
        ax = fig.add_subplot(gs[i, j])

        im = ax.contourf(X, Y, boundary_fields[i_cell].reshape(resolution, resolution), cmap="jet", levels=100)
    ...
    vmax = 0.833333
    cbar.set_ticks([0.00, vmax/2, vmax])
    cbar.ax.tick_params(labelsize=18)
    plt.savefig('../../figs/Frontiers/figure2D.png', dpi=300)